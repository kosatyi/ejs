...
const compileTemplates = ({namespace, filename}) => {
    ...
    return new Transform({
        ...
        transform(file, enc, callback) {
            if (file !== null) {
                if (file.isStream()) {
                    this.emit('error', new Error('compileTemplates: Streaming not supported'));
                    callback();
                    ...
                    babelTransform(compile(contents,relative).source).then((result)=>{
                        const data = {
                            name: file.relative,
